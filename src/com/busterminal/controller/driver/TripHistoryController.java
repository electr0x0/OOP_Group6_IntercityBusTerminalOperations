...
public class TripHistoryController implements Initializable {
    ...
    ArrayList<BusTripSchedule> bustripList = new ArrayList();
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       bustripList = RelationshipDatabaseClass.getInstance().getAllAvailableTripSchedules();
    ...
    private void viewDetailsOnClick(ActionEvent event) throws IOException {
       ...
       d2.add(dm);
        ...
        TripHistoryDetailController p = someLoader.getController();
         for(BusTripSchedule x: bustripList){
             for(DummyTripHistory y: d2){
                 if(x.getScheduleId() == y.getScheduleId()){
                     String time = x.getTime();
                     String bus = x.getTrip().getBus().getManufacturer();
                     String fleet = x.getTrip().getFleetType();
                     String distance =Double.toString(x.getTrip().getDistance());
                     String fare = Integer.toString(x.getTrip().getAdultFare());
